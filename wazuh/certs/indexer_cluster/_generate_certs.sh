...
echo "create: dashboard2-key-temp.pem"

openssl genrsa -out dashboard2-key-temp.pem 2048

echo "create: dashboard2-key.pem"

openssl pkcs8 -inform PEM -outform PEM -in dashboard2-key-temp.pem -topk8 -nocrypt -v1 PBE-SHA1-3DES -out dashboard2-key.pem

echo "create: dashboard.csr"

openssl req -days 3650 -new -key dashboard2-key.pem -out dashboard2.csr -subj "/C=US/L=California/O=Company/CN=dashboard"

echo "create: dashboard.pem"

openssl x509 -req -days 3650 -in dashboard2.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out dashboard2.pem

echo "copy: dashboard.pem"

mv dashboard2-key.pem ../dashboard_http/key.pem
mv dashboard2.pem ../dashboard_http/cert.pem
cp root-ca.pem ../dashboard_http/root-ca.pem
